  ...
  Widget build(BuildContext context) {
    ...
    if (element.attributes.containsKey('src')) {
      ...
      if (element.children.length > 0) {
        element.children.forEach((dom.Element source) {
          try {
            if (source.attributes['type'] == "video/mp4") {
              videoUrl = source.attributes['src'];
            }
          } catch (e) {
            print(e);
          }
    ...
    return new NetworkPlayerLifeCycle(
      videoUrl, (BuildContext context, VideoPlayerController controller) =>
